...
public class CombustibleController {
    ...
    public ResponseEntity<ApiResponse<CombustibleDTO>> crear(@Valid @RequestBody CombustibleDTO json){
        try{
            ...
            ApiResponse<CombustibleDTO> respuesta = new ApiResponse<>(false, "Error crítico: "+ e.getMessage(), null);
    ...
    public ResponseEntity<ApiResponse<List<CombustibleDTO>>> obtenerTodos(){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<List <CombustibleDTO>> respuestaError = new ApiResponse<>(false , "No se pudieron obtener los datos" , null);
    ...
    public ResponseEntity<ApiResponse<CombustibleDTO>> obtenerPorId(@PathVariable Integer id){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<CombustibleDTO> respuestaError = new ApiResponse<>(false , "No se pudo completar la búsqueda del id " + id , null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
    ...
    public ResponseEntity<ApiResponse<CombustibleDTO>> eliminar(@PathVariable Integer id) {
        try {
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<CombustibleDTO> respuestaError = new ApiResponse<>(false , "Error inesperado, consulte con el administrador para solucionar el problema", null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
