...
public class DetallemantenimientoController {
    ...
    public ResponseEntity<ApiResponse<DetallemantenimientoDTO>> crear(@Valid @RequestBody DetallemantenimientoDTO json){
        try{
            ...
            ApiResponse<DetallemantenimientoDTO> respuesta = new ApiResponse<>(false, "Error crítico: "+ e.getMessage(), null);
    ...
    public ResponseEntity<ApiResponse<List<DetallemantenimientoDTO>>> obtenerTodos(){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<List <DetallemantenimientoDTO>> respuestaError = new ApiResponse<>(false , "No se pudieron obtener los datos" , null);
    ...
    public ResponseEntity<ApiResponse<DetallemantenimientoDTO>> obtenerPorId(@PathVariable Integer id){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<DetallemantenimientoDTO> respuestaError = new ApiResponse<>(false , "No se pudo completar la búsqueda del id " + id , null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
    ...
    public ResponseEntity<ApiResponse<DetallemantenimientoDTO>> eliminar(@PathVariable Integer id) {
        try {
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<DetallemantenimientoDTO> respuestaError = new ApiResponse<>(false , "Error inesperado, consulte con el administrador para solucionar el problema", null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
