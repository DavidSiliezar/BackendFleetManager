...
public class TallerController {
    ...
    public ResponseEntity<ApiResponse<TallerDTO>> crear(@Valid @RequestBody TallerDTO json){
        try{
            ...
            ApiResponse<TallerDTO> respuesta = new ApiResponse<>(false, "Error crítico: "+ e.getMessage(), null);
    ...
    public ResponseEntity<ApiResponse<List<TallerDTO>>> obtenerTodos(){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<List <TallerDTO>> respuestaError = new ApiResponse<>(false , "No se pudieron obtener los datos" , null);
    ...
    public ResponseEntity<ApiResponse<TallerDTO>> obtenerPorId(@PathVariable Integer id){
        try{
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<TallerDTO> respuestaError = new ApiResponse<>(false , "No se pudo completar la búsqueda del id " + id , null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }

    @GetMapping("/nombre/{nombre}")
    public ResponseEntity<ApiResponse<TallerDTO>> obtenerPorNombre(@PathVariable String nombre){
        try{
            TallerDTO dto = service.buscarPorNombre(nombre);
            if (dto != null){
                ApiResponse<TallerDTO> respuestaExitosa = new ApiResponse<>(true , "Dato encontrado" , dto);
                return ResponseEntity.ok(respuestaExitosa);
            }
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<TallerDTO> respuestaError = new ApiResponse<>(false , "No se pudo completar la búsqueda del nombre " + nombre , null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
    ...
    public ResponseEntity<ApiResponse<TallerDTO>> eliminar(@PathVariable Integer id) {
        try {
            ...
            log.error("Error crítico, consulte con el administrador");
            ApiResponse<TallerDTO> respuestaError = new ApiResponse<>(false , "Error inesperado, consulte con el administrador para solucionar el problema", null);
            return  ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(respuestaError);
        }
    }
